...
const useDaysPagination = (initialDate: DateTime, minDate: DateTime, maxDate: DateTime, selectedDate: DateTime) => {
    ...
    return {
        items: [
            { page: undefined, type: "previous", selected: false, disabled: datesAreSame(datesToShow[0], minDate) },
            ...
            { page: undefined, type: "next", selected: false, disabled: datesAreSame(datesToShow[datesToShow.length - 1], maxDate) || datesAreSame(halfDate, maxDate) }
        ],
        onClickNext,
        onClickPrevious
    }
